...
// - Entity Name: user.
// - Entity ID: 102019abe83be8a743aa9a6be3435513d32.
...
export class UserORM {
  ...
  private constructor() {
    ...
    this.entityId = '102019abe83be8a743aa9a6be3435513d32';
  ...
  /**
   * Get user by DisplayName index
   * This function gets data by index.
   */
  async getUserByDisplayName(
    display_name: string,
  ): Promise<UserModel[]> {
    const index = createIndexDisplayName(
      display_name
    );
  ...
  /**
   * Set (update) user by DisplayName index
   * This function replaces data, so the data must be complete.
   * Must keep `id`, `data_creator`, `create_time` unchanged as original data (as fetched).
   * DO NOT SET `data_updater` and `update_time` since backend will fill it automatically.
   * It will respond the set record, with `data_updater` and `update_time` filled by backend.
   */
  async setUserByDisplayName(
    display_name: string,
    data: UserModel
  ): Promise<UserModel[]> {
    const index = createIndexDisplayName(
  ...
  /**
   * Delete user by DisplayName index
   */
  async deleteUserByDisplayName(
    display_name: string
  ): Promise<void> {
    const index = createIndexDisplayName(
      display_name
...
/**
 * Create index for DisplayName fields
 */
function createIndexDisplayName(
  display_name: string
): Index {
  const values: Value[] = [
